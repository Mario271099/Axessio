...
14. **`next lint` supprimé de Next 16** → le lint passe par la CLI ESLint (`npm run lint` = `eslint .`, config flat dans `eslint.config.mjs`, `eslint-config-next` 16 exporte des flat configs natives). Étape lint branchée dans `ci.yml`. La règle `react-hooks/set-state-in-effect` est volontairement en **warning** (8 occurrences legacy documentées — patterns `mounted`/localStorage) : ne pas en ajouter de nouvelles, ne pas la repasser en error sans refactor préalable.
15. **Staging indexable par Google** → Vercel ne pose PAS de `X-Robots-Tag: noindex` sur un domaine custom rattaché à une branche de preview (staging.axessyo.com). Triple garde-fou basé sur `IS_PRODUCTION_DEPLOYMENT` (`src/lib/site.ts`, `VERCEL_ENV === "production"`) : header `X-Robots-Tag` (next.config.ts), `robots.txt` disallow all, meta robots noindex (layout racine). Toute nouvelle surface publique hérite automatiquement du comportement.
